...
<div class="fixed-row">
    <div class="app-title">
        <div class="active-wrap">
            <h1><i class="fa fa-file"></i> Edit Job</h1>
    ...
    <div class="row section-mg row-md-body no-nav">
        <div class="col-md-8 mx-auto">
            <div class="tile">
                <div class="tile-body">
                    <form action="{{ route('admin.job.detail.update') }}" method="POST" role="form">
                        @csrf
                            ...
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control @error('jobTypeId') is-invalid @enderror" name="jobTypeId" id="jobTypeId">
                                        ...
                                        <option value="{{$category->id}}" {{($category->id === $job->jobTypeId)? 'selected' : ''}}>{{$category->title}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input class="form-control @error('valid_till') is-invalid @enderror" type="date" name="valid_till" id="valid_till" value="{{ $job->valid_till }}"/>
                                    @error('valid_till') {{ $message ?? '' }} @enderror
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">Job Name<span class="m-l-5 text-danger"> *</span></label>
                                <input class="form-control @error('name') is-invalid @enderror" type="text" id="name" name="name" value="{{ $job->name }}"/>
                                @error('name') {{ $message }} @enderror
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label" for="name">Location <span class="m-l-5 text-danger"> *</span></label>
                            <input class="form-control @error('location') is-invalid @enderror" type="text" name="location" id="location" value="{{ $job->location }}"/>
